# (stomataUpper & StomataShrub --> StomataNaturalVeg) & StomataC3Grass --> StomataC3NaturalVeg
...
StomataNaturalVeg <- function(i) {
  ...
  cs[i] <- 0.5 * (cs[i] + co2conc - anl / gbco2u)
  cs[i] <- max (1.05 * gamstar, cs[i])
  
  # calculate new value of gs using implicit scheme
  
  gs[i] <- 0.5 * (gs[i] + (coefm[i] * anl * rh12 / cs[i] + coefb[i] * stresst))
  ...
  csc <- max (1.05 * gamstar, co2conc - an[i] / gbco2u)
  # calculate diagnostic canopy average stomatal conductance (big leaf approach)
  gsc <- coefm[i] * an[i] * rh12 / csc + coefb[i] * stresst
  gsc <- max (gsmin[i], coefb[i] * stresst, gsc)

  # TODO: verificar, so tem na C3Grass ??
