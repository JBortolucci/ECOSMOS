...
SoybeanCROPGRO <- function(iyear, iyear0, imonth, iday, jday, index) {
  ...
  if (croplive[i]==1) {
    ...
    if(DAS==1){
      ...
      PHENOL (iyear, iyear0, jday, DAS,DYNAMIC, index) 
      ...
      #-----------------------------------------------------------------------
      DEMAND(DYNAMIC,DAS , CROP, PAR, PGAVL,RPROAV, TAVG, index)
      ...
      #-----------------------------------------------------------------------
      if (CROP != 'FA') {
        INCOMP(DYNAMIC, index)
      }
      
      #-----------------------------------------------------------------------
      GROW(DYNAMIC,iyear,jday, ISWNIT,ISWSYM, index)
      
      #-----------------------------------------------------------------------
      NUPTAK(DYNAMIC, index)
      
      #-----------------------------------------------------------------------
      MOBIL(DYNAMIC, index)
      
      #-----------------------------------------------------------------------
      NFIX(DYNAMIC, DAS, CNODMN, CTONOD, index) 
      
      #-----------------------------------------------------------------------
      PODS(DYNAMIC, DAS, NAVL,ISWWAT,iyear,jday,PGAVL, index)
      
      #-----------------------------------------------------------------------
      VEGGR (DYNAMIC,DAS,iyear,jday,CMINEP,CSAVEV,NAVL,PAR,PG,PGAVL, index)
      
      #-----------------------------------------------------------------------
      #     Call leaf senescence routine for initialization
      #-----------------------------------------------------------------------
      SENES(DYNAMIC,DAS,PAR, index)
      
      #-----------------------------------------------------------------------
      #     Call to root growth and rooting depth routine
      #-----------------------------------------------------------------------
      ROOTS(DYNAMIC,CROP,ISWWAT, index)
    ...
    if (DAS == NVEG0) {
      ...
      #-----------------------------------------------------------------------
      GROW("EMERG",iyear,jday, ISWNIT,ISWSYM, index)
      plai[i]  <- max(XLAI,0.01) # Henrique: atribui IAF inicial para ECOSMOS (2020-10-1)
      
      #-----------------------------------------------------------------------
      #     Call to root growth and rooting depth routine
      #-----------------------------------------------------------------------
      ROOTS("EMERG",CROP,  ISWWAT, index)
      
      #-----------------------------------------------------------------------
      #       DYNAMIC = EMERG (not INTEGR) here
      DEMAND("EMERG", DAS, CROP, PAR, PGAVL,RPROAV, TAVG, index)
      
      #-----------------------------------------------------------------------
      PODS("EMERG", DAS, NAVL,ISWWAT,iyear,jday,PGAVL, index)
      #-----------------------------------------------------------------------
      VEGGR ("EMERG",DAS,iyear,jday, CMINEP, CSAVEV,   NAVL,  PAR, PG, PGAVL, index)
      
      #-----------------------------------------------------------------------
    }
    #----------------------------------------------------------------------
    if (DETACH == 'Y' & DAS <= NVEG0+1) {
      PODDET("EMERG", iyear, jday, index)
    }
    ...
    if (DAS >= NVEG0) {
      ...
      #-----------------------------------------------------------------------
      if (DETACH == 'Y' & DAS >= NR1) {
        PODDET(DYNAMIC, iyear, jday, index)
      }
