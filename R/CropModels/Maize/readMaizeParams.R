...
readMaizeParams <-function(pathExcel = "a",filePath = "BRFRM047", simInstances = new.env() ,column = "BR0001", simId = 1, i = 1){
  ...
  simInstances[[simId]]$plantList[[i]]$params[["gddsfrac"]] 	   <- as.numeric(data[which(data$`VAR#` %in% column)[1],8])
  simInstances[[simId]]$plantList[[i]]$params[["laic"]] 	       <- as.numeric(data[which(data$`VAR#` %in% column)[1],9])
  simInstances[[simId]]$plantList[[i]]$params[["laim"]] 	       <- as.numeric(data[which(data$`VAR#` %in% column)[1],10])
