...
source("R/CropModels/Rice/RiceGrowthPheno.R")
source("R/CropModels/Rice/RicePlanting.R")
source("R/CropModels/Rice/RiceCropresidue.R")


# TODO: remover index depois, passar as variáveis automaticamente.
RiceModel <- function(year, month, day, index) {
  ...
  #' Por exemplo:
  #' 
  #' Indica que o modelo RicePlanting está dentro de RiceModel.
  #' AddSubModel(RicePlanting, RiceModel)
  #'
  
  b1 <- plantList$rice$params$b1 # não usando
  b2 <- plantList$rice$params$b2 # não usando 
  
  
  environment(RicePlanting)    <- env
  environment(RiceGrowthPheno)   <- env
  environment(RiceCropresidue) <- env
  
  depth <- numeric(nsoilay)
  
  # new michel
  # beta1[index] <- min(0.982 + idpp[index] * 0.00002, 0.995)
  beta1[index] <- min(0.995*(1-exp(-0.1*idpp[index])), 0.995) # funcao que altera vmax_pft do Rice em funcao do tempo
...
#   assign("froot", froot, envir = env)
  ...
  RicePlanting(year0, year, month, day, jday, ffact, index)
  
  RiceGrowthPheno(year, year0, month, day, jday, index)
  
  RiceCropresidue(year, year0, jday, index)
