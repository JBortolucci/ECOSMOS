...
OilPalmPlanting <- function(iyear0, iyear, imonth, iday, jday, ffact, index) {
  ...
  # if(imonth == 1 & iday == 25 & (iyear == 2003 | iyear == 2004)) {
  #   browser()
  # }
  ...
  if (exist[i] == 1 && croplive[i] != 1 && cropplant[i] == 0) {
    ...
    if(cropy == 0 && cdays >= pstart[i] && cdays <= (pstart[i]+180)) {
      ...
      print(paste0('1st Plant Oil palm: ', as.Date(paste(iyear, jday, sep = '-'), format = '%Y-%j')))

      nstrleaves <- 7
      cbiolini <- (laitrans / specla) / (nstrleaves * 4)

      leaves <- data.frame("gdd"    = seq(1, 0, -1/nstrleaves)[2:(nstrleaves+1)] * mxgddsen,
                           "islive" = rep(1, nstrleaves),
                           "dm"     = seq(nstrleaves, 1, -1) * cbiolini,
                           "dpp"    = rep(0, nstrleaves),
                           "index"  = seq(nstrleaves-1, 0, -1),
                           "dry"    = rep(0, nstrleaves),
                           "n"      = rep(0, nstrleaves))

      fruits <- data.frame("ssi"    = rep(0, nstrleaves),
      					           "hrvday" = rep(0, nstrleaves),
      					           "dm"     = rep(0, nstrleaves),
      					           "dpp"    = rep(0, nstrleaves),
      					           "dmmax"  = rep(0, nstrleaves))
