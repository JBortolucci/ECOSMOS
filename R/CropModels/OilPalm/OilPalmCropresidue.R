...
OilPalmCropresidue <- function (year, year0, jday, index) {
  ...
  # if(croplive[j] == 1) {
  #   falll <- (Deadleaves) * deltay/kg_C_M2_to_T_ha
  #   fallr <- (Deadcoroots + Deadfineroots) * deltay/kg_C_M2_to_T_ha  
  #   fallw <- (Deadwood) * deltay/kg_C_M2_to_T_ha + DBranch_decay
  # }
  ...
  if(exist[j] == 1 && harvdate[j] == jday) {
    ...
    # falll <- falll +  cbiol[j] 
    # 
    # fallr <- fallr + cbior[j] + cbiocr[j]
    # 
    # fallw <- fallw + cbiow[j] * (1 -fyield[j]) + cbiob[j] + DBranch_attached
    ...
    # totts   <- 1
    # 
    print('cropy = 0 start plant again for Oil Palm')
    cropy <- 0
