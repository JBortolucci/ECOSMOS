...
FOR_PHENOL <- function (iyear, iyear0, jday, DAS, DYNAMIC, TAVG, ISWWAT, index){
  ...
  SDAGE <-  150  # ! SDAGE = PAGE     Transplant age (days)
  ATEMP <-  32   #   ATEMP = PENV
  
  # TODO: SDAGE, ATEMP -  Colocar no essas variáveis no Template?? Leandro 17/11/2020
  ...
  if (DYNAMIC == 'SEASINIT') {
    ...
    if (NVEG0 > DAS) {
      ...
      if(ISWWAT == 'Y') {
        ...
        FT[1]  <- CURV(CTMP[1],TB[K],TO1[K],TO2[K],TM[K],TAVG) # TODO: Verificado a variavel TGROAV, TGROAV = TAVG
      } else {
        FSW[1] <- 1.
        FT[1]  <- CURV(CTMP[1],TB[K],TO1[K],TO2[K],TM[K],TAVG) # TODO: Verificado a variavel TGROAV, TGROAV = TAVG
      }
  ...
  assign("YRPLT",YRPLT, envir = env)
  assign("PHZACC",PHZACC, envir = env) # TODO: Lembrete - ASSIGN estava no RSTAGE.
