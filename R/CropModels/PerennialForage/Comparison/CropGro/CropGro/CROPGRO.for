...
C-----------------------------------------------------------------------
      ...
      ! print *, DAS, CMINEP, WCRST , WCRRT , WCRSH , WCRLF
      PGAVL = PG + CMINEP
      write(586,*) PGAVL
      ! print *,DAS, CMINEP, CMOBMX, DTX, DXR57, WCRST, WCRRT, WCRSH, WCRLF
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     &  ShutMob, RootMob, ShelMob)                        !Output
      ...
      ELSE
c       !PCNVEG = -99.    !Wait for GBuild fix for -99''s
         PCNVEG = 0.
        ENDIF
...
!-----------------------------------------------------------------------
      ENDIF
!-----------------------------------------------------------------------
      write(584,*)DAS,PAR,PG,AGEFAC,MAINR,EOP,RO,RP,TRWUP,TURFAC,SWFAC, CMINEP, TAVG
      write(590,*)DAS,ST
      write(591,*)DAS,SW
      write(592,*)DAS,NO3
      write(593,*)DAS,NH4
      ...
      write(585,*) DAS, VSTAGE, RSTAGE, DTX, DXR57, FRACDN,      !PHENOL & RSTAGES
     &  PGAVL, NAVL, TURFAC, SWFAC,                              !CROPGRO
     &  CMINEP, CNODMN, CTONOD, CTONODR,
     &  RSPNH4, RSPNO3, CGRSD, CGRSH, CSAVEV,
     &  AGRSD1, AGRSD2, AGRVG, AGRVG2, CDMREP, F, FNINL,  !DEMAND (PHTIM, PNTIM removidos)
     &  FNINR, FNINS, FNINSD, FRLF, FRRT, FRSTM, GDMSD,
     &  GRRAT1, NDMNEW,  NDMOLD, NDMREP, NDMSDR, NDMTOT,
     &  NDMVEG, NMINEP, NMOBR, POTCAR,
     &  POTLIP, SDGR, TURADD, XFRT, YREND,               
     &  SWIDOT, WLFDOT, WSHIDT, WTNFX,                    !GROW 
     ...
     &  CADLF, CADST, CANHT, CANWH, CMINEA, CRUSLF,       !VEGGR
     &  CRUSRT, CRUSSH, CRUSST, EXCESS, NADLF, NADRT,   
     &  NADST, NGRLF, NGRRT, NGRST, NSTRES,            
     &  TNLEAK, WLDOTN, WRDOTN, WSDOTN,
     &  AGRSD3, LAGSD, LNGPEG, NGRSD, NGRSH, PCTMAT,      !PODS (SDNO, SHELN, WTSD, WTSHE, FLWN removidos | POTCAR, POTLIP, SEEDNO da GROW)       
     &  PODNO, SDVAR,
     &  SHVAR, WSDDTN, WSHDTN, WTABRT, WTSHMT,
     &  NMINEA, NRUSLF, NRUSRT, NRUSSH, NRUSST,           !MOBIL
     &  RTDEP, SATFAC, SRDOT,                             !ROOTS (SENRT & RLV removidos)
     &  SLDOT, SLNDOT, SSDOT, SSNDOT,                     !SENES
     &  CNOD, NDTH, NFIXN, NODGR,                         !NFIX (DWNOD, DWNODA, WTNFX, já na GROW | SENNOD removido)
     &  TRNU, TRNH4U, TRNO3U                              !NUPTAK
