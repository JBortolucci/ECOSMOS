...
source("R/CropModels/PerennialForage/ForagePlanting.R")
source("R/CropModels/PerennialForage/ForageCROPGRO.R")
source("R/CropModels/PerennialForage/ForageCropresidue.R")
...
ForageModel <- function(year, month, day, index) {
  ...
  environment(ForageCROPGRO)     <- env
  environment(ForageCropresidue) <- env
  ...
  ForageCropresidue(year, year0, jday, index)
