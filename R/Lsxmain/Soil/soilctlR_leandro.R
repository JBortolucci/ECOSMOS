...
soilctlR_leandro <- function() {
  ...
  # hsoi !wsoi hydraul suction bex ibex bperm dtime !porosflo !fwtop !fwpud rhow wflo poros !wisoi !wpud 
  fsqueez <- soilh2o (owsoi, porosflo, wsoi, fwtop, fwpud)
  # fsqueez <- soilh2o (owsoi, fsqueez, porosflo, wflo, wsoi, fwtop, fwpud)
  ...
  # wflo ch2o  !tsoi hsoi consoi csoi rhosoi rhow poros !wisoi
  # retornoHeat <- soilheat (otsoi, owsoi, c0pud, fhtop, c1pud, wflo, tsoi)
  ...
  assign("grunof", grunof, envir = env)
  assign("fwtop", fwtop, envir = env)
  assign("wisoi", wisoi, envir = env)
  assign("wsoi", wsoi, envir = env)
  assign("fwpud", fwpud, envir = env)
  assign("soihfl", soihfl, envir = env)
  assign("gadjust", gadjust, envir = env)
  assign("porosflo", porosflo, envir = env)
  assign("gdrain", gdrain, envir = env)
  assign("wipud", wipud, envir = env)
  assign("wpud", wpud, envir = env)
...
# soilh2o <- function(owsoi, fsqueez, porosflo, wflo, wsoi, fwtop, fwpud) {
soilh2o <- function(owsoi, porosflo, wsoi, fwtop, fwpud) {
  ...
  # hsoi wsoi hydraul suction bex ibex bperm dtime porosflo fwtop fwpud rhow wflo poros wisoi wpud fwpud
  ...
  retornoTridia <- tridia (nsoilay, d1,d2,d3, rhs, wsoi, w1,w2)
  wsoi <- retornoTridia
...
soilheat <- function(otsoi, owsoi, c0pud, fhtop, c1pud) {
  ...
  tsoi <- tridia (nsoilay, d1,d2,d3, rhs, tsoi, w1,w2)
