...
c
     ...
     >        iholdsoiln,    ! 0: dont save inorganic soil N values 1: keep inorganic soil N values 
...
c
      if(iyranom .le. iyrlast+nrun) then
         call inird('input/temp.CRU.1901_2000.US.nc',istyrm)
         if (istyrm .le. 0) istyrm = 9999
      end if
c
      if (iyrdaily .le. iyrlast+nrun) then
         call inird('input/prate_us.fanom.1948_2002.nc',istyrd)
...
c sant      write(225,*) 'Corrente Year !!!!!!!!!!!!!!!!!!!! ',iyear
c
c reset julian date
c
         jday = 0
...
c
            if (iyear .lt. imetyear .or. iyear .gt. imetend) then
               ...
               call rdcru2 (imonth, iyear, iyear0, iwest, jnorth)

c sant - o ctl estava errado-  call rdcru_interpola(imonth,iyear,iyear0,iwest,jnorth)
            endif
c
...
c sant- start planting depending on the numbers of ratoons
...
c sant- start planting for all points at the same time
...
c**************************** NCEP Anomaly (+ Station) ****************************
     ...
     >         (iyear.eq.imetend.and.jday.gt.dmetend))  ) then
                  if(jday.eq.1) print*,'reading daily NCEP anomaly'
                  
                  call rdday   (jday, imonth, iyear, istyrd, nanomd, iwest, jnorth)
...
c	                  if(npoi.eq.1) then
c                       call rdstation  (jday,imonth,iyear, istyear)
c	                  else
...
c***************************** Hourly Data ****************************************
c sant            else if (iyear   .ge. imetyear .and.
c sant     >               iyear   .le. imetend  .and.
c sant     >               iday    .eq. 1        .and.
c sant     >               imonth  .eq. 1        ) then

c sant              call dailymet (imonth, iday, seed, jday)

c*****************************    CRU Data       *********************************
               elseif(iyear.lt.imetyear.or.iyear.gt.imetend.or. 
     >         (iyear.eq.imetyear.and.jday.lt.dmetyear).or.
     >         (iyear.eq.imetend.and.jday.gt.dmetend) ) then  

	               if(jday.eq.1) print*,'using montly CRU'

                  if (iyear.ge.cdcyear.and.iyear.le.2006)
     >               call rdcdc (jday,imonth,iyear,cdcyear,iwest,jnorth) !CRU Data (+ CDC prec)
...
c	                  endif	
                  ...
                  call daily (iyear, imonth, iday, jday, seed, 0, iyranom, iyrlast, nrun) !Only CRU Data
...
c sant- when start rotation irotations will be called at the beginning of the planting year! stead of normal callendar
...
c                  plen = min (plenmax, int (plenmin + ran2(seed) * (plenmax-plenmin+1) ))
                  plen = min (plenmax, int (plenmin + 0.5 * (plenmax-plenmin+1) ))
               else
                  plen = 1
               endif
c
...
c 
               plens   = dtime * plen
c               startp  = dtime * min (niter-plen, int(ran2(seed)*(niter-plen+1)))
               startp  = dtime * min (niter-plen, int(0.5*(niter-plen+1)))
               endp    = startp + plens
c
...
c sant                call inimet (iyear,imonth,iday,nday,time,seed)
...
c
c sant-  write out vars to validate agains flux tower (FOR THE POINT 1, TOP LEFT GRID POINT)
...
c sant - should be *1e6*0.432 to convert from mol_CO2.m-2.s-1 to Kg_C.ha-1.dtime or *43.2 to Kg_C.m-2.s-1    
...
c sant         write (*,*) ' '
c sant         write (*,*) 'writing monthly output'
c sant         write (*,*) ' '
