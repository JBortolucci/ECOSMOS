...
c
      ...
      call const (gdd12, npoi, 0.0)
      call const (gddsgcp, npoi*2, 0.0)	
...
c
        ...
        gdd12(i) = gdd12(i) + max(0.0, min(dtemp  +273.16-baset(16), 30.0))

csant- old scheme specific for 1 and half year....
c	if(pmmin(npoi,16).le.7) then
c	   if(k.ge.pmmin(npoi,16).and.k.le.(mod(pmmin(npoi,16)+4,12)+1)) then
c	   print*,k
c         gddsgcp(i,1)= gddsgcp(i,1) + max(0.0, min(dtemp+273.16-baset(16),30.0))
c           endif
c	else
c	   if(k.ge.pmmin(npoi,16).and.k.le.12) then
c	   print*,k
c         gddsgcp(i,1)= gddsgcp(i,1) + max(0.0, min(dtemp+273.16-baset(16),30.0))
c           endif
c	   if(k.ge.1.and.k.le.pmmin(npoi,16)-7) then
c 	   print*,k
c           gddsgcp(i,1)= gddsgcp(i,1) + max(0.0, min(dtemp+273.16-baset(16),30.0))
c	   endif
c	endif
c
	...
	if(pmmin(i,16).lt.pcm(i,16)-2) then

	   if(k.ge.pmmin(i,16).and.k.lt.pcm(i,16)-2) then  
c	   print*,k
         gddsgcp(i,1)= gddsgcp(i,1) + max(0.0, min(dtemp+273.16-baset(16),30.0))
           endif

	else if(pmmin(i,16).gt.pcm(i,16)-2) then
...
c	   print*,k
	   ...
	   if(k.ge.1.and.k.lt.pcm(i,16)-2) then
c 	   print*,k
           gddsgcp(i,1)= gddsgcp(i,1) + max(0.0, min(dtemp+273.16-baset(16),30.0))
	   endif
...
c     
 ...
 210    continue
	...
	print*,i,'initial gddsgcp(i,1)/gddsgcp(i,2)',gddsgcp(i,1)/gddsgcp(i,2)

c
 200  continue


c        gdd0(i) = gdd0(i) + max(0.0, dtemp)
c        gdd5(i) = gdd5(i) + max(0.0, (dtemp - 5.0))
c        gdd0c(i) = gdd0c(i) + max(0.0, min(dtemp  +273.16-baset(15),  26.0))
c        gdd8(i)  = gdd8(i)  + max(0.0, min(dtemp  +273.16-baset(14),  30.0))
c        gdd10(i) = gdd10(i) + max(0.0, min(dtemp  +273.16-baset(13), 30.0))
c        gdd12(i) = gdd12(i) + max(0.0, min(dtemp  +273.16-baset(16), 30.0))
c
...
c == SV. Cuadra ==
c grassland are currently applyed everywhere
c
          if (ipast .gt. 0.0) then  
            do j = 1, 16
              exist(i,j) = 0.0
 	    enddo
	   exist(i,11) = 1.0
...
c
          icropsum(i) = isoybean +imaize +iwheat +isgc +irotation
c
...
c
c 14) c4 crop - sugarcane
c
          if (isgc .eq. 1)              exist(i,16) = 1.0
c


csant (exist) - if apply bare soil everywhere
c            do j = 1, npft
c              exist(i,j) = 0.0
c          enddo
