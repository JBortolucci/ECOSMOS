...
c   - c4 crops          :  maize and sugarcane (type 14, 16)  
c
...
csant- these functions can be set to 0 to compare productivity
csant- (have to adjust these functios, they have a great impact in biomass production) 
c ---------------------------------------------------------------------
...
c
        tnpp(i,16)  = nppdummy(i,16)                         -
     >               rroot * cbior(i,16)            * funcb
...
c
c ---------------------------------------------------------------------
      subroutine sumday (istep, plens, iyear, jday)
c ---------------------------------------------------------------------
c
c common blocks
c
...
c
     ...
     >   (iyear.eq.imetend.and.jday.gt.dmetend) ) ) print *,
     >      'ERROR in sumday:', i, jday, adrain(i) + adsnow(i), precip(i)
...
c
        amtotnleach(i) = ((nmtimes-1) * amtotnleach(i) +
     >                    fout(i,5)/dtime * 1e+04 * 86400.) * rwork
        amno3leach(i)  = ((nmtimes-1) * amno3leach(i) +
     >                    nout(i,5)/dtime * 1e+04 * 86400.) * rwork
c
c ---------------------------------------------------------------------
...
c
       ...
       rnet(i) =  ( (nmtimes-1)*rnet(i)  + ( solad(i,1) * (1 - asurd(i,1)) + 
     >	solad(i,2)*(1 - asurd(i,2)) + solai(i,1)*(1 - asuri(i,1)) +
     >           solai(i,2)*(1-asuri(i,2)) +  fira(i) - firb(i) )   )* rwork
...
c
     ...
     >                  tnpp(i,15) * rwork3) * rwork
        amnpp(i,16) = ((nmtimes-1) * amnpp(i,16) +
     >                  tnpp(i,16) * rwork3) * rwork
c
...
c
     ...
     >                  tgpp(i,15) * rwork3) * rwork
        aygpp(i,16) = ((nytimes-1) * aygpp(i,16) +
     >                  tgpp(i,16) * rwork3) * rwork
c
...
c
     ...
     >   aygpp(i,13)+ aygpp(i,14) + aygpp(i,15) + aygpp(i,16)
c
c ---------------------------------------------------------------------
...
c
     ...
     >                  tnpp(i,15) * rwork3) * rwork
        aynpp(i,16) = ((nytimes-1) * aynpp(i,16) +
     >                  tnpp(i,16) * rwork3) * rwork
c
...
c
     ...
     >  aynpp(i,13) + aynpp(i,14) + aynpp(i,15) + aynpp(i,16)
c
c ---------------------------------------------------------------------
...
c
      ...
      write (*,*) ' '
      write (*,*) '* * * corrent simulation year - ',iyear,'  * * *'
      write (*,*) ' '
      write (*,9001) gnee
      write (*,9000) gnpp
      write (*,9002) ggpp
