...
c                    rndnum = 1.0 + (float(ndaypm(imonth)) - 1.0) * ran2(seed)
                    rndnum = 1.0 + (float(ndaypm(imonth)) - 1.0) * 0.5
...
c
c          ud(i) = 1.13989 * eud * (-log(ran2(seed)))**0.30 
          ud(i) = 1.13989 * eud * (-log(0.5))**0.30 
c
...
c
          ...
          sphumid = humidfrac * qsat(esat(273.16+xint(i,imonth)),psurf(i))
c
...
c
c sant 9001 continue ! I didnt find the reason why have to back all, instead of just rain lengh calculations
c
...
c sant- these is an first attempt to acheive some agrement between the rad from station data and from IBIS (A. Friend)
c sant- though it looks working, one should come up with some publish solution in near future!! 	
	if(time.eq.0.) then
...
c select random wind speed following this equation
c
c        ua(i) = 1.13989 * ud(i) * (-log(ran2(seed)))**0.30 
        ua(i) = 1.13989 * ud(i) * (-log(0.5))**0.30 
...
c        trans = cloud(i) / sw     ! cloud(i) is surface insolation
        trans = cloud(i) / (sw*coszen(i)) !csant - incidente solar radiation
...
c select random wind speed following this equation
c
c jk     ua(i) = 1.13989 * ud(i) * (-log(ran2(seed)))**0.30 
c
c fix wind speeds to always be above 2.5 m/sec and below 10.0 m/sec
c
