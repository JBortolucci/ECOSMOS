c Last updated 10.04.02 C. Kucharik
c
c #####      #     ####    ####   ######   ####    ####   #    #  ######  #    #
c #    #     #    #    #  #    #  #       #    #  #    #  #    #  #       ##  ##
c #####      #    #    #  #       #####   #    #  #       ######  #####   # ## #
...
c
c --------------------------------------------------------------------------
      subroutine soilbgc (iyear,iyear0,imonth,iday,jday,nspinsoil,
     >                    spin,spinmax)
c --------------------------------------------------------------------------
c
      include 'implicit.h'
...
c
     ...
     >          cnfroot     ! input c:n ratio of root litter turnover
*    >          rconst,     ! value set to 1200.  from Verberne model 
*    >          fmxcpool        ! maximum fraction allowed in metabolic pool
c 
...
c      kpb = 0.000055            !adapted from Arlington model 
...
c model discussion (Geoderma, December 1997 special issue).  fmxcpool is the
...
c
c
      cnleaf  = 60.0      ! average c:n ratio for leaf litterfall
      cnfroot = 80.0      ! average c:n ratio for fine root turnover 
      cnwood = 200.0      ! average c:n ratio for woody debris

c
c total timesteps (daily) used to divide litterfall into daily fractions 
c for natural vegetation.  Crops are added as a pulse input with totts = 1
c
       totts=1./float(ndaypy)
c
c check to see if crops are planted - if so, modify c:n ratios 
c crop residue is input on the harvest date
     ...
     >         harvdate(i,j) .eq. jday) then
             ...
             cnfroot = max(60.0, cnroot(i,j))   ! calculated in crop residue at harvest 
...
csant- included here textcls and associated vars...
csant- test after change lmin to lmin(i,j), set at initial.f, in order to redice time (processing), and check if gives result 
          lmin = textcls (msand,mclay) 
csant- for the USR lmin differ from the global soil map
csant-	lmin = 6  it doesnt have any impact in short therm simulations, test in future for 100 years. 

c
csant          domtexti = nint(domtext(i,kk))
...
c
c if carfrac is greater than 0.4, set fbpom = 0.7, if carfrac is less
c than 0.17, set fbpom = 0.30 (sandy soil)
c
c        fbpom = min(max(0.3, carfrac/0.4 * 0.7),0.7)      
         fbpom = 0.50
c
c ------------------------------------------------------------------------
...
c	if(i.eq.1)print*,iyear,jday,croplive(i,16),fallr(i), falll(i)
...
c --------------------------------------------------------------------------
c
c           deposn(i)    = (0.21  + 0.0028 * (precip(i)*0.1))*1.e-3
c           fixsoin(i)   = (-0.18 + 0.14   * (precip(i)*0.1))*1.e-3
c
            deposn(i)    = (0.0005753  + 0.0028 * (precip(i)*0.1))*1.e-3
c
c 7-06-05 CJK : modify rates for the US based on Simon's dataset that adjusts
c the rates each year (from actual data - 1940-1999)
c
            if (iyear .ge. 1940 .and. iyear .le. 1999) deposn(i) = deposn(i) *
     >                           ndepfact(i,iyear+1-1940) 
            if (iyear .gt. 1999) deposn(i) = deposn(i) *
     >                           ndepfact(i,60) 
c
c          if (iyear .ge. 1901) deposn(i) = deposn(i) *
c     >                           ndepfact(i,60) 

            fixsoin(i)   = (-0.0004932 + 0.14   * (precip(i)*0.1))*1.e-3
c
c From Landsberg and Gower, 1997
c changes cjk 5-21-01 : research on rates of asymbiotic nitrogen fixation
c which would be characterized more by IBIS than symbiotic, are about 1-2% of
c the symbiotic potential.  So, adjusting fixsoin by that ratio.
c also maximum allowable annual fixsoin is now 30 kg N ha-1 based on
c Landsberg and Gower, 1997
c
c typical asymbiotic n-fixation by natural vegetation ranges from 0.1 -6
c kg N ha-1 in decidous and coniferous (temperate and boreal forests) to
c 20 in tropical forests.
c
