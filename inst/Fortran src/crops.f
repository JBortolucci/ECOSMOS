...
c sant- These numbers are now at params.crp
c
...
c sant- have to make                   if (j .eq. 15) whtdop(i,iyear)   = jdaythese numbers extern
c sant      ptemp(13)   = 284.16          ! soybean minimum 10 day average temperature for planting (K)
c sant      ptemp(14)   = 283.16          ! maize minimum   10 day average temperature for planting (K)
c sant      ptemp(16)   = 283.16          ! sugarcane minimum   10 day average temperature for planting (K)
c sant      pmintemp(13)= 279.16          ! soybean minimum 10 day average min temp for planting (K) 
c     pmintemp(14)= 277.16          ! maize minimum 10 day average min temp for planting (K) 
c sant      pmintemp(14)= 279.16          ! maize minimum 10 day average min temp for planting (K) 
c sant      pmintemp(16)= 279.16          ! sugarcane minimum 10 day average min temp for planting (K) 
c sant      pmmin(13)   = 5               ! soybean earliest month to plant (month)
c sant      pmmin(14)   = 4               ! maize earliest month to plant (month)
c sant      pmmin(16)   = 4               ! sugarcane earliest month to plant (month)
c sant      pdmin(13)   = 1               ! soybean earliest day in earliest month to plant (day)
c sant      pdmin(14)   = 1               ! maize earliest day in earliest month to plant (day)
c sant      pdmin(16)   = 1               ! sugarcane earliest day in earliest month to plant (day)
c
c spring wheat planting date requirements
c note: iwheat = 1 (spring), iwheat = 2 (winter) 
c
c sant      if (iwheat .eq. 1) then
c sant        ptemp(15)   = 280.16        ! spring wheat minimum 10 day average temperature for planting (K) 
c sant        pmintemp(15)= 272.16        ! spring wheat minimum 10 day average min temp threshold for planting (K)
c sant        pmmin(15)   = 4             ! spring wheat earliest month to plant (month)
c sant        pdmin(15)   = 1             ! spring wheat earliest day in earliest month to plant (day)
c
...
c
c sant      else if (iwheat .eq. 2) then  ! winter wheat
c sant        pmintemp(15) = 278.16       ! average 5-day minimum temperature needed to plant winter wheat (~ 40 F) 
c sant        pmmin(15)    = 9            ! earliest month in which winter wheat is planted (month)
c sant        pdmin(15)    = 1            ! earliest day (of earliest month) in which winter wheat is planted (day) 
c sant        pmmax(15)    = 11           ! latest possible month to plant winter wheat (month)
c sant        pdmax(15)    = 30           ! latest day in latest month to plant winter wheat (day) 
c sant      endif
c

c sant-- externally ,ptemp(16),pcm(16),pcd(16),cgrain(16),fnopt(16)


        
      do 100 i = 1, npoi

c sant- write half second year prec and temperature to see possible relationship with produc. 
	      if(jday.eq.1) then
	         cropout(i,7,31) =  td(i)
...
c sant- write anual prec (half first to half second years)and temperature to see possible relationship with produc. 
	      if(jday.eq.182) then
...
c s      ant- write half first year (must displace down output in this case) prec and temperature to see possible relationship with produc. 
	      if(jday.eq.182) then
...
c 
c sant- Put a if to make distinct the kind of cultures and regions (ill be inplementaded transforming the pdim and pmmin in 2d vars)
            if  ((j.eq.13 .or. j.eq.14. or. j.eq.16) .or.
     >      (j .eq. 15 .and. iwheat .eq. 1))  then 
c

c sant- find the minimum day of planting relative to the planting calendar.
c sant- .and.cdays(i).lt.367 ->  planting only after planting year start. 
...
c sant- I put after harvest, ok?  idop(i,j)      = 999
               pdate(i,j)     = 0.0
...
c sant-              pdate(i,j)     = 0.0
c sant-              idop(i,j)      = 999
c
c for continuous, annual winter wheat type crop
c sant - to passing in the if above the wheat shoul not be eq to 1??
...
c
c sant - differentiate tropical and subtropical cultures and sugarcane (or maybe annual and peren in future) .
c*********************************************************************************
...
c sant - WHY ask here if imonth and iday ge. the minimum date, because this is asked above
c sant- that is cropplant.eq.0 only if this condition, of date, is attained. right??
...
c
     ...
     >             cdays(i)         .ge. pstart(i,j))    then     ! impose earliest planting date 
                     ...
                     if (j .eq. 15) whtdop(i,iyear)   = jday
c
c sant                gddmaturity(i,13) = max(950., min (gddcorn(i,iyear-1), hybgdd(j)))  ! assign hybrid based on last year 
...
c
                 if     (j .eq. 13) then 
                   sumhy(i,j) = sumhy(i,j) + gddsoy(i,iy)  !c sant - gddcorn(i,iy) * 0.8
                   sumdp(i,j) = sumdp(i,j) + float(soydop(i,iy))
                 elseif (j .eq. 14) then 
...
c sant- to avoid that a crop that had be killed by frost be planted again in the same year. 
c              jday             .eq. idop(i,j)    .and.               
c     >        gdd8(i)          .ge. gddmin(j))    then         ! impose limit on growing season length needed
...
c
     ...
     >         a10tmin(i)       .gt. pmintemp(j)  .and. 
     >         cdays(i).ge.pstart(i,j).and.cdays(i).le.(pstart(i,j)+180) )then     ! impose earliest planting date 
c sant- cdays.lt.pstar+90 is necessary to not deslocate the crop cycle, and harvest in summer, for example. 
c sant- and also to avoid planting at the end of the last crop year - like cdays = 307 in the start of the year
...
c sant- should we cut off this  or it can be usefful in future
c
...
c sant- as we dont have a historical data set, assuming constant.
      if(idpp(i,j).eq.1.and.j.eq.16) then
         fertnitro(i,16)= 0.025   !kg_n m-2 y-1 or 100 Kg/ha
c sant	print*,'     fertnitro   ',i,j,jday,fertnitro(i,16) 
...
c sant- for a first guess, I've put 0.12 for planting and 0.03 for ratoon (but need be adjusted)
c sant- (based on lat 27, lon -81; and 45 days for planting and 15 days of delay for plt and ratoon, Modeling...pfd/fig.1)
...
c sant- why are these parameters reset here??
...
c sant- not all, specially at the end of the season (future put some function here)
                  grnfraccrop(i,j) = 1.0  
c
...
c sant- I've changed leafout for instance, just to have more external control during the test phase.
                  if(j.eq.16) then
...
c            else if ((hui(i,j) .ge. huigrain(j) .or. idpp(i,j) .ge. mxdgfi(j)) 
                  ...
                  else if (hui(i,j) .ge. huigrain(j).and.j.ne.16 .and. 
     >               croplive(i,j) .eq. 1.) then   !c sant- dont need croplive here (it is in the croplive if!)

c sant- now the control is "only" internal, through the gddmaturity, not all internal as we control gddmat.
...
c sant-the max(gddplant(i,j)-huigrain(j),0.0)  is necessary in case of idpp(i,j) .ge. mxdgfi(j)
c sant- however, now is comented  elseif ((hui(i,j) .ge. huigrain(j) .or. idpp(i,j) .ge. mxdgfi(j)) 
c
...
c sant- scheme based on CANEGRO model (Singels et al. 2005) 
c sant - if sugarcane is planted, it takes long to construct the root sistem, if ratoon leafs develop faster
                     if(cropy(i).eq.1) then
...
c asnt- make sure that aleaf(j) is at least .eq. to aleaff(j) 
                     astem(j) = min( max(0.,aerial(j)-aleaff(j)) ,astem(j) ) 
                     aleaf(j) = aerial(j)-astem(j) 

c sant*****************  leaf/Stalk allocation **************************************************************
c sant- Adjust the leaf/Stalk allocation in function os Temperature (physiological effect)

c sant. all these relations, propoused below, have to be tested !!!
c sant- the original, however I limited it to start after a certain stalk size!
c sant- using these if the temp. effect is almos half dumped - 

c       if(rm(i).gt.50.) then
                     astem(j) = astem(j) + min( aleaf(j) ,
     >                  (astem(j)/aerial(j))*ldf*aleaf(j)*min(1.,exp(tmld*ecf7)/exp((td(i)-273.16)*ecf7) ) )

c sant: see if in future I can validate this relation.
c sant:        astem(j) = astem(j) + ( plai(i,j)/laimx(j) )*
c sant:       ldf*aleaf(j)*min(1,exp(tmld*ecf7)/exp((td(i)-273.16)*ecf7) )
...
c sant- for sugarcane astem is the structural carbon, and arepr is the sucrose carbon
...
c sant- Adjust the Sucrose/Stalk allocation in function os Temperature (physiological effect)
...
c sant- astem já é q4(da planilha),  ou seja, o resto do astem-sucrose, ou - astem(j)= astem(j)- arepr(j)
...
c**************************************************************************************************
c sant -     STAR of leaf adjust  -> adjust aleaf; check for LAI max and apply the leaf turn over **
c**************************************************************************************************
...
c sant-               if (peaklai(i,j) .eq. 0) then
...
c sant-                    aleaf(j) = min(1.0 - aroot(j), laimx(j) -
c sant-     >                        plai(i,j)) / (specla(j) * adnpp(i,j)) 
c sant- talk with Chris, and if its correct I'v to change in the annual crops (above) and in the others versions of the code
...
c sant                     peaklai(i,j) = 1
                     endif  ! laimx


c if maize - employ leaf expansion routine based on a modified CERES gdd approach
c sant- I clean the code comented here (but keeps in the annual crops), but in future 
c sant- should be usefull take a look and considere these approuche, or look for other specific for sugarcane
c sant- I remove here, lai decline is apllyed in the carbon and after the new - adnpp - is added in the cbiol below 
c sant                   plai(i,j)    = plai(i,j) + (specla(j) * aleaf(j)
c sant     >                          * max(0.0, adnpp(i,j)))

c sant-               endif !of peak lai
...
c sant__________________________ LAI DECLINE __________________________________

c sant_old_decai- LAI decline is based on the LAI at the initial of the phase 3 
c sant_old_decai-	if(rm(i).gt.laidcs) then 

c sant_old_decai-              if (thrlai(i,j) .lt. 0.0001) 
c sant_old_decai-     >         thrlai(i,j) = plai(i,j)
c
                     templai(i,j)   = (cbiol(i,j)*specla(j))

c sant_old_decai-                plai(i,j) = max(thrlai(i,j) * (1.0-min(max((gddplant(i,j)-
c sant_old_decai-     >                      huigrain(j)),0.0)/(0.55*gddmaturity(i,j)), 1.0)
c sant_old_decai-     >                      **laicons(j)), xminlai)

c sant-   *****   New scheme to be tested in future ******* 
c sant- LAI decline is exponential proportional to the LAI (in the sence that if LAI increases de the decline is hight) 
c sant- the ( exp((plai(i,j)-laidc)/laidc) / exp((laimx(j)-laidc)/laidc) ) ranges exponentially from 0 (LAI=0) to 1 (LAI=laimx) 
c sant. the idea is that if LAI aproaches 5 or 6, the shadon will be great, increase the mortality
c sant. however, I have to get some data sets to validate this hypothesis
...
c sant- as Iam not using the exp(..plai) function, I pass the lai decline direct to the cbiol (as for root)
c sant-   *****   New scheme to be tested in future ******* 

c sant- test the APSIM (parametrization) - lai declines linear for temp lower than 10C till zero if td = 0C
...
c san_old_decai-	 endif !LAI decline
c sant___________________ END of the new LAI decline __________________________________ 

c**************************************************************************************************
c sant -                  END of leaf adjusts  
c**************************************************************************************************
...
c sant	cropout(i,j,32)=aroot(j)
                  cropout(i,j,33)=arepr(j)
c sant	cropout(i,j,34)=astem(j)
...
c sant. use this expression when the decay is applyed on LAI, instead of Cbiol
                  cbiol(i,j) = cbiol(i,j) + aleaf(j) * max (0.0, adnpp(i,j))
     >               - (laidecl(i,j)/specla(j))  
c sant. use the eq. bellow for tauleaf acting in the carbon or the above for in LAI
c sant. if laidecl=taulaf*plai, the result should be the same.
...
c sant-to calculate the root decay today (see below)
...
c sant- this is just a first attempt to consider dead leaves attached (that is important for sugarcane)
c sant- next step should be find a real observed value, and try to specified the 
c sant -relfectance transmitta for brown leaves (specified in radiation.f)
...
c sant- the months here keeped in the harvest season window and prevent the shift of the harvest season 
c sant- (ex. if always harv. ocurre at maximun date, after 4 ratoon the harv. will be 4th months late than the 1th)	
...
c     >              idpp(i,j) .ge. mxmat(j)) then
     ...
     >	               .and.   idpp(i,j).ge. (mxmat(j)-30)) then
                        ...
                        grnfraccrop(i,j)  = 0.0           ! turn all vegetation to brown
                        if (harvdate(i,j) .eq. 999) harvdate(i,j) = jday 
c sant- but, as harvdate is eq jday, it will be equal to 0.001 at cropupdate, right?
                        plai(i,j)         = 0.25          ! simulates remaining stubble/mulch
                     endif
...
c sant- this carbon "doesnt exist" - is only to avois erros, right? 
c sant  ------(and forgot to sum cbios!!!)cropout(i,j,30)
c sant          biomass(i,j) = cbiol(i,j) + cbiog(i,j) + cbior(i,j)
c sant-     >                   + cbiow(i,j)
c
...
c
*      maxhi(13)  = 0.38 ! soybean (from Cabelguenne et al. 1999) / c sant = 0.5 , (Yield components, dry matter, LAI and LAD...PDF)
c
...
c sant - segundo o Humberto as folhas permanecem atachadas,       falll(i) = falll(i) + falllsgc(i,3)  !decay today   
...
c	
c sant- as I pass idpp, I can treck witch year if(idop(i,j).lt.367)  pdate(i,j) = idop(i,j)
...
c calculate crop yield in bu/acre or if sugarcane in ton/ha of fresh weight
	if(j.eq.16)then
c sant- fyield= 0.8 :lost of top and botton at harvest 
c sant-  1/0.3 = 30% of DM related to the Fresh weight 
c sant- 10 =  kg/m2 to tonne(t)/ha
c sant-    cgrain(16)=0.42105 ?? to convert 1g of C to Xg of C12_H22_O11 (C=12;H=1;O=16g/mol)
c sant - 1.07 -to consider around 7 % of non C chemical substances (minerals, starch, protein and other metabolites)
...
c sant-     falll(i) = falll(i) + cbiol(i,j) + (cbios(i,j)+cbiog(i,j))*(1-fyield(j))  !leaf + meristem and base not harvested
...
c for corn/maize
c sant- I think that is best not use nitro stress for now, but after the model is ready include again.
