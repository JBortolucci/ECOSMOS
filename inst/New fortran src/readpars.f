c modified by C. Kucharik on 10.04.02
c
c #####   #####    ##    #####   #####     ##    #####    ####   
c #    #  #       #  #   #    #  #    #   #  #   #    #  #      
c #    #  #      #    #  #    #  #    #  #    #  #    #   ####       
...
c 
      ...
      include 'compar.h'    ! npoi, npft, scpft, ecpft
c     include 'comage.h'    ! npftu
      ...
      include 'comcrop.h'   ! Crop parameters
      include 'comnitr.h'   ! N cycling parameters
c  
c Local variables
c      
	...
	real rratoon,pmminrd(npft),pdminrd(npft),pcmrd(npft),pcdrd(npft)
...
c      call readitem(parm_unit, parm_file, dummyvar)
c
c      npftu2 = nint(dummyvar)
c      if (npftu2 .ne. npftu) then
c        write (*,9004) parm_file, npftu2, npftu
c        goto 9006 ! In the circumstances this seems the best thing to do! 
c      end if
c9004  format ('RD_PARAM Warning: Number of upper canopy (tree) PFTs ',
c     >        'in ', A10, ' is: ', 
c     >        I2, ' number in comage.h is: ', I2)  
c
...
c      call readitem(parm_unit, parm_file, chifuz)
c      call readitem(parm_unit, parm_file, chiflz)
...
9032  format (' Number of soil types in comtex.h is: ', I2)  
     ...
     >         dummyvar, dummyvar)
      ...
      call readitems(parm_unit, parm_file, 3,
     >         fmxcpool, rconst, h20, dummyvar, dummyvar,
     >         dummyvar, dummyvar, dummyvar, dummyvar, dummyvar)
...
c ******************************************************************************
c open the parameter file 'params.crp' for input; read in crop parameters...

      parm_file = 'params.crp'
      open(UNIT=parm_unit, FILE=parm_file, STATUS='OLD', ERR=9001)
      do j = scpft, ecpft 
        call readitems(parm_unit, parm_file, 8, 
     >           lotemp(j), hitemp(j), 
     >           drought(j), f1(j), f2(j), 
     >           baset(j), mxtmp(j), tkill(j),
     >           dummyvar, dummyvar) 
      end do
      
      do j = scpft, ecpft 
        call readitems(parm_unit, parm_file, 10, 
     >           laicons(j), allconsl(j), 
     >           allconss(j), laimx(j), arooti(j), 
     >           arootf(j), aleaff(j), astemf(j),
     >           declfact(j), fleafi(j)) 
      end do

      do j = scpft, ecpft 
        call readitems(parm_unit, parm_file, 6, 
     >  ptemp(j),pmintemp(j),pmminrd(j),pdminrd(j),
     >  pcmrd(j),pcdrd(j),dummyvar,dummyvar,
     > dummyvar,dummyvar) 
      end do
      ...
      do j = scpft, ecpft 
        call readitems(parm_unit, parm_file, 8, 
     >           hybgdd(j), gddmin(j), 
     >           mxgddgf(j), mxdgfi(j), mxmat(j), 
     >           lfemerg(j), grnfill(j), bfact(j),
     >           dummyvar, dummyvar) 
      end do

csant- make sure that the end of the crop year will be after harvest
        do j = scpft, ecpft 
		do i=1,npoi
	...
	if(j.eq.16) then
	   pcm(i,j)=mod(pmmin(i,j)+(mxmat(j)/30)-1,12.)+2
	   pcd(i,j)=pdmin(i,j)

      print*,'point ',i,' sugarcane crop year star/ends - month:',pcm(i,16),' day: ',pcd(i,16)
	print*,' '
	...
	enddo
      ...
      do j = 1, 2 ! number of wheat crop types 
        call readitems(parm_unit, parm_file, 7, 
     >           grnwht(j), fleafiwht(j), 
     >           mgddgf(j), mxmatwht(j), fnlfmxw(j), 
     >           fngrmxw(j), fnoptw(j), dummyvar,
     >           dummyvar, dummyvar) 
      end do
      ...
      call readitem(parm_unit, parm_file, rratoon)
...
csant- sugarcane allocation parameters
	...
	nratoon = rratoon
      ...
      do j = scpft, ecpft 
        call readitems(parm_unit, parm_file, 10, 
     >           cgrain(j), convfact(j), 
     >           maxhi(j), fyield(j), cfrac(j), 
     >           fnlfmx(j), fngrmx(j), sratio(j),
     >           rratio(j), fnopt(j)) 
      end do
