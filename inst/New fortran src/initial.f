...
c initialize station data vars
      call const (stintd,    npoi, -999.0)
      call const (stinqd,    npoi, -999.0)
      call const (stinprecd, npoi, -999.0)
      call const (stinwindd, npoi, -999.0)
      call const (stincldd,  npoi, -999.0)
      call const (stintmax,  npoi, -999.0)
      call const (stintmin,  npoi, -999.0)
      call const (stinrad,   npoi, -999.0)
c
...
c
      ...
      call const (amtotnleach,npoi, 0.0) 
...
c
      do 300 k = 1, nsoilay 
        do 310 i = 1, npoi 
c
csant          domtexti = nint(domtext(i,k))
c
c         domtexti = 12 
c
...
csant-	testing the impact of different soil class
c	lmin = 6 			!class more similar to the site observations
csant	 print*,'teste soil for all layers = 9',i,k,'lmin',lmin
c
csant          if (domtexti .eq. 1) then
csant            lmin = 1
csant          else if (domtexti .eq. 2) then
csant            lmin = 2
csant          else if (domtexti .eq. 3) then
csant            lmin = 3
csant          else if (domtexti .eq. 4) then
csant            lmin = 5
csant          else if (domtexti .eq. 5) then
csant            lmin = 5
csant          else if (domtexti .eq. 6) then
csant            lmin = 4
csant          else if (domtexti .eq. 7) then
csant            lmin = 6
csant          else if (domtexti .eq. 8) then
csant            lmin = 8
csant          else if (domtexti .eq. 9) then
csant            lmin = 7
csant          else if (domtexti .eq. 10) then
csant            lmin = 9
csant          else if (domtexti .eq. 11) then
csant            lmin = 10
csant          else if (domtexti .eq. 12) then
csant            lmin = 11
csant          endif
...
c porosity (fraction):
	  if(i.eq.1) print*,k,fracsand(i,k),fracclay(i,k)
c 
...
c
	...
	if(pedofunct.eq.1) then !use pedo functions transfer equations instead of table values
...
c porosity (fraction):
c      
          poros(i,k) = (50.5-3.7*fclay-14.2*fsand)/100
ccoriginal          poros(i,k) = porosdat(lmin)

c "b" exponent for the Campbell moisture-release equation:
ccoriginal          bex(i,k) = bexdat(lmin)
c nearest integer of "b" exponent (for computational efficiency):
ccoriginal          ibex(i,k) = nint(bex(i,k))
        bex(i,k) = 3.10 + 15.7 * fclay - 0.3 * fsand 
        ibex(i,k) = nint(bex(i,k))

c saturated matric (air entry) potential (m-h2o):
          suction(i,k) = 10**(2.17-0.63*fclay-1.58*fsand)*0.01
ccoriginal      suction(i,k) = suctiondat(lmin)
c
c saturated hydraulic conductivity (m s-1):
ccorignal           hydraul(i,k) = hydrauldat(lmin)   

	
     	if(ipast.eq.1) then
        hydraul(i,k) = 0.25 * 10 ** (-0.6 - 0.64 * fclay + 1.26 * fsand)
     >                * 0.0254 / (3600)
	print*,'Considerando compactadação do solo para pastagem'
	else
        hydraul(i,k) = 1.0 * 10 ** (-0.6 - 0.64 * fclay + 1.26 * fsand)
     >                * 0.0254 / (3600)
...
c field capacity (defined relative to the porosity):
ccoriginal          sfield(i,k) = 1.0 / poros(i,k) * sfielddat(lmin)
c
        sfield(i,k) = 1 / poros(i,k) * (50.5-3.7*fclay-14.2*fsand)/100
     >               * (1.157e-9 / hydraul(i,k))**(1/(2*bex(i,k)+3))

c
c wilting point (defined relative to the porosity):
ccoriginal          swilt(i,k)  = 1.0 / poros(i,k) * swiltdat(lmin)
        swilt(i,k) = 1 / poros(i,k) * (50.5-3.7*fclay-14.2*fsand)/100
     >            *((10**(2.17-0.63*fclay-1.58*fsand)*0.01)
     >            /(1500/9.80665))**(1/bex(i,k))
...
c
csant- talk with Chris to send this routine to io.f  
c ---------------------------------------------------------------------
...
c
      if (irestart .eq. 0) then
        ...
        call constint (harvdate,   npoi*npft, 999)
      endif
c
...
c
c	
csant	inital atmosphere height, after first time step it is updated in canopy.f/subroutine canini(jday)
        za(i) = 100.0

c specify canopy height parameters
c calculated as a function of only the vegetative fraction of each grid cell
c TET changed 6/26/02, CSANT changed 2011
        if (icropsum(i) .eq. 0.0) then  !csant
        zbot(i,1) =  0.05
	...
	do ij = 1,16
	print*,'point = ',i,' PFT-> ',ij,'  exist = ',exist(i,ij)
	enddo

	print*,'initial canopy height for point ',i,  ztop(i,2),zbot(i,2),ztop(i,1),zbot(i,1)	

	else
	
csant. for crops ztop is calculated in crops.f 
        zbot(i,1) =  0.01
        ztop(i,1) =  0.50
c
c        zbot(i,2) =  2.0 
...
csant-        ztop(i,2) =  20.0 
	...
	zbot(i,2) =  4.5  !to match the tower, wind

	   if(zbot(i,2).le.ztopmxsgc) then
	   print*,'upper canopy botton cannot be lower than maximum crop height'
	   stop
	   endif
...
c
CSANT -for the global crop model,have to be specified for each culture..
